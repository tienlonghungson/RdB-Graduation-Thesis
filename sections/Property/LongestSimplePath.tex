...
\begin{proof}[Proof for lemma~\ref{lem:upperbound}]
    ...
    % $(2)$ It can be shown that for each $u,v\in\mathscr{R}$ such that neither $u$ nor $v$ is the end vertex of the last edge of $\mathscr{P}$, paths $\mathcal{P}_{u}$ and $\mathcal{P}_{v}$ are edge-disjoint.
